              ...
              {formateurs.map((f) => {
                ...
                return (
                 <tr key={f._id}>
                    ...
                    <td><input type="number" min="1" max="5" value={evalData.comportement || ""} onChange={(e) => handleChange(f._id, "comportement", e.target.value)} /></td>
                    <td style={{ backgroundColor: "#ff4444", color: "white", fontWeight: "bold" }}>{calculateScore(evalData)}</td>
                    <td><button onClick={() => saveEvaluation(f._id)}>Enregistrer</button></td>
                    <td>
