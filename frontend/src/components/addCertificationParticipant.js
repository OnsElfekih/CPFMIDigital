...
import "./addCertificationParticipant.css";

const AddCertificationsParticipant = () => {
  ...
  if (selectedDate < today) {
    ...
    try {
      await axios.post("http://localhost:3001/certifications/addcertifParticipant", formData);
      setSuccess("Certification du participant est ajoutée avec succès");
      setFormData({
        nomSociete: "",
        nomPrenomPart: "",
        theme: "",
  ...
  return (
    <>
      ...
      >
        <h2>Ajouter une certification pour un participant</h2>
        ...
        <form onSubmit={handleSubmit} className="certification-form">
          <div>
            ...
            <input
              type="text"
              name="nomPrenomPart"
              placeholder="Nom et Prénom du Participant"
              value={formData.nomPrenomPart}
              onChange={handleChange}
              required
            />
          </div>
...
export default AddCertificationsParticipant;
